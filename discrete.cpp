...
public:
    unsigned int bm;
    int cnt;
    elem(unsigned int bmm=0,int cntt=0)
    {
        bm=bmm;
        cnt=cntt;
    }
...
int countcell(unsigned int bm)
{
    int i,j;
    int ret=0;
    bool tbm[500][500];
    //for(i=0;i<numcells;i++)for(j=0;j<numcells;j++)tbm[i][j]=false;
    memset(tbm,false,250000*sizeof(bool));
    for(i=0;i<numdps;i++)
    {
        if(((1<<i)&bm)!=0)
        {
            for(j=0;j<clists[i].size();j++)
            {
                pair<int,int> pii=clists[i][j];
                if(tbm[pii.first][pii.second]==false)
                {
                    ret++;
                    tbm[pii.first][pii.second]=true;
                }
            }
        }
    }
...
vector<int> subset(vector<int> s)
{
    int i,j;
    unsigned int mask;
    int sz=s.size();
    vector< pair<unsigned int,int> > mc;
    mc.clear();
    for(i=1;i<(1<<sz);i++)
    {
        mask=0;
        ...
        //cout<<mask<<endl;
        mc.push_back(pair<unsigned int,int>(mask,countcell(mask)));
        //cout<<countcell(mask)<<endl;
    }
    vector<int> ret;
    ret.clear();
...
void init(){
    ...
    for(i=0;i<numcells;i++)for(j=0;j<numcells;j++)
    {
        unsigned int tbm;
        tbm=cells[i][j].vs;
        if(tbm!=0)
        {
            ...
            if(it==mp.end())
            {
                mp.insert(pair<unsigned int,int>(tbm,1));
            }
            else mp[tbm]+=1;
        }
    }
...
//    }
    ...
    cout<<"Time Difference: "<<t2-t1<<" miliseconds"<<endl;
