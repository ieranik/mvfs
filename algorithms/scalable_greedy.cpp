...
void init(){
    ...
    //for(i=0;i<numdps;i++)fprintf(fout,"%d\n",cellcount[i]);
    //fprintf(fout,"\n");

    //cout<<cellcount[i]<<endl;
    ...
    for(i=0;i<choice.size();i++)
    {
        ...
        fprintf(fout,"%.3lf\n",rai*csize*csize);
        //cout<<rai*csize*csize<<endl;
    }
    //for(i=0;i<numcells;i++)for(j=0;j<numcells;j++)if(ra[i][j]==true)rai++;
    ...
    cout<<endl<<"Time Difference: "<<end_time-begin_time<<" miliseconds"<<endl;
